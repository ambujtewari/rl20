...
JMLR 2002 \
The authors begins by noting the E<sup>3</sup> does not use optimism in the face of uncertainty principle (goes back at least to the <a href="https://doi.org/10.1613/jair.301">1996 survey</a> by Kaelbling, Littman and Moore where it is discussed in Section 2.2 titled "Ad-hoc techniques"). It explicitly separates exploration from exploitation. They suggest that such a separation possibly does not work in adversarial settings and instead rely on implicit exploration: states that have not been visited enough are given the maximum possible reward R<sub>max</sub> (hence the name of the algorithm) in a fictitious MDP. Following an optimal policy in the fictitious MDP has an interesting and useful propery in the real MDP: it is always either optimal or it leads to efficient learning.
____
Sham Kakade
